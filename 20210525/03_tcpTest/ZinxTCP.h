...
public:
	virtual ZinxTcpData *CreateTcpDataChannel(int _fd) = 0;//_fd 为请求连接的客户端
};

/* tcp监听通道类，这是一个实体类（不建议继承该类），开发者可以直接创建tcp监听
 * 通道对象，一般地，开发者应该在该类的构造函数中，指定一个tcp套接字通道类的
 * 工厂类，当有连接到来后，该工厂类的成员方法会被调用
 */
...
private:
	unsigned short m_usPort = 0;//端口
	int m_fd = -1;//socket
	IZinxTcpConnFact *m_ConnFac = NULL;//对象
};
#endif
