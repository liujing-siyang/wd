...
bool ZinxTCPListen::Init()
{
	...
	if (0 <= iListenFd)
	{
		...
		stServaddr.sin_addr.s_addr = htonl();
...
bool ZinxTCPListen::ReadFd(std::string & _input)
{
	...
	iDataFd = accept(m_fd, (struct sockaddr *)&stClientAddr, &lAddrLen);//阻塞等待客户端的连接请求，整个程序启动后就阻塞在这
	if (0 <= iDataFd)
	{
		auto poTcpDataChannel = m_ConnFac->CreateTcpDataChannel(iDataFd);
		if (NULL != poTcpDataChannel)
		{
			bRet = ZinxKernel::Zinx_Add_Channel(*poTcpDataChannel);//将连接成功的通道加入
		}
	}
	else
	{
...
bool ZinxTcpData::ReadFd(std::string & _input)
{
	...
	while (0 < (iReadLen = recv(m_DataFd, acBuff, sizeof(acBuff), MSG_DONTWAIT)))//非阻塞
	{
		bRet = true;
